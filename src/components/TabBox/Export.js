...
export const Export = () => {
  ...
  useEffect(() => {
    ...
    if (currentLayerID === "1") {
      ...
      return fetch(
        `https://zebra.geodb.host/user/admin/api/v2/sql?q=SELECT ST_AsText(the_geom), ${columnNames} FROM ${maps[mapID].layers[currentLayerID].carto_tableName}`
      )
        .then((resp) => resp.json())
        .then((response) => {
          for (var i = 0; i < response.rows.length; i++) {
  ...
  useEffect(() => {
    if (showSettlements === true) {
      ...
      if (currentCountry[currentLayerID].query && mapID) {
        ...
        return fetch(
          `https://zebra.geodb.host/user/admin/api/v2/sql?q=SELECT ST_AsText(the_geom), ${columnNames} FROM ${maps[mapID].layers["4"].carto_tableName}`
        )
          .then((resp) => resp.json())
          .then((response) => {
            for (var i = 0; i < response.rows.length; i++) {
  ...
  useEffect(() => {
    if (userData && mapID && columnNames) {
      ...
      for (var i = 0; i < userData.length; i++) {
        fetch(
          `https://zebra.geodb.host/user/admin/api/v2/sql?q=SELECT ST_AsText(the_geom), ${columnNames}  FROM ${maps[mapID].layers[currentLayerID].carto_tableName} WHERE ST_Intersects(ST_SetSRID(ST_Point(${userData[i].Longitude}, ${userData[i].Latitude}), 4326), ${maps[mapID].layers[currentLayerID].carto_tableName}.the_geom)`
        )
          .then((resp) => resp.json())
          .then((response) => {
            // for (var i = 0; i < response.rows.length; i++) {
